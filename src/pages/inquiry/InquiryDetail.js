  ...
  const fetchInquiry = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        // Store quotation ID if available
        if (inquiryData.quotation && inquiryData.quotation._id) {
          setQuotationId(inquiryData.quotation._id);
        }
        ...
        setInquiry({
          ...
          quotation: inquiryData.quotation ? {
            id: inquiryData.quotation._id,
            quotationNumber: inquiryData.quotation.quotationNumber
          } : null
        });
      } else {
              ...
              <div className="flex space-x-3">
                ...
                {inquiry?.status === 'quoted' && !isEditing && (
                  <div className="flex space-x-3">
                    ...
                    {(quotationId || inquiry?.quotation?.id) && (
                      <button
                        onClick={() => handleDownloadQuotationPDF(quotationId || inquiry?.quotation?.id, inquiry?.quotation?.quotationNumber)}
